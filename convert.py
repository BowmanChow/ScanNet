...
for scene in scene_list:
    ...
    for label_dir in label_dir_list:
        label_dir_full = scene_path / label_dir
        print(f"converting {label_dir_full = }")
        output_color_dir = scene_path / f"{label_dir}-color"
        output_color_dir.mkdir(exist_ok=True)
        label_img_list = os.listdir(label_dir_full)
        label_img_list.sort()
        for label_img_file in label_img_list:
            label_img = Image.open(label_dir_full / label_img_file)
            label_img_np = np.array(label_img)
            # print(f"{label_img_np.shape = }")
            # print(f"{label_img_np.dtype = }")
            print(f"{label_img_file = } {label_img_np.max() = }")
            # print(f"{label_img_np.min() = }")
            label_img = Image.fromarray(label_img_np.astype(np.uint8))
            label_img.putpalette(_palette)
            label_img.save(output_color_dir / label_img_file)

# label = Image.open("./scene0000_00/scene0000_00_2d-label-filt/label-filt/0.png")

# label_np = np.array(label)
# print(f"{label_np.shape = }")
# print(f"{label_np.dtype = }")
# print(f"{label_np.max() = }")
# print(f"{label_np.min() = }")

# label = Image.fromarray(label_np.astype(np.uint8))

# label.putpalette(_palette)
# label.save("output.png")